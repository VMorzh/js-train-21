...
class Guitarist extends Musician {
  #band; // Об'являємо приватне поле #band;
  // Конструктор приймає три параметри: name, instrument та band
  constructor(name, instrument, band) {
    super(name, instrument); // Виклик конструктора батьківського класу з двома параметрами name, instrument
    this.#band = band; // Присвоюємо вхідне значення band до приватного поля #band
  }
...
Object.defineProperties(Musician.prototype, {
  band: {
    set(newBand) {
      this.band = newBand;
    },
...
class Concert extends Performance {
  ...
  constructor(band, location, date, ticketPrice) {
    // Використовуємо ключове слово super для виклику конструктора базового класу (Performance)
